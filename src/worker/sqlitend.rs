...
impl SQLiteStatements {
    pub fn prepare_next(&mut self) -> Result<Option<SQLitePreparedStatement>> {
        ...
        Ok(Some(SQLitePreparedStatement { sql, sqlite3, stmt }))
    }
    ...
    pub fn get_one(&self) -> Result<Option<SQLiteStatementValues>> {
        ...
        for col_ndx in 0..column_count {
            let (column_name, column_type) = unsafe {
                ...
                if ptr.is_null() {
                    return Err(SQLitendError::GetColumnName("null pointer".into()));
                }
                let Ok(column_name) = CStr::from_ptr(ptr).to_str() else {
                    return Err(SQLitendError::GetColumnName("not a string".into()));
                };
            ...
            let value = unsafe {
                match column_type {
                    ...
                    SQLITE_INTEGER => sqlite3_column_int64(self.stmt, col_ndx).to_string(),
                    ...
                    SQLITE_BLOB => {
                        ...
                        format!("x'{}'", hex::encode(slice))
                    }
                    _ => return Err(SQLitendError::UnsupportColumnType(column_type)),
                }
            };

            column.push(column_name.to_string());
            row.push(value);
        }
